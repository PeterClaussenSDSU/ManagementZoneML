...
We will use previously processed yield data (see `ProcessFiles.Rmd` and `CombiningMultipleYears.Rmd`). These data, comprising 5 years of harvest data from a single cropland over the period from 2013 to 2018 (excepting 2014), are saved in an R data file. We will be using `Yield` estimates in $bushels/acre$ and the associated spatial coordinates `Easting` and `Northing` in meters from the origin at the lower left corner of the cropland. `Easting` and `Northing` are metric projects derived from the original GPS latitude and longitude.


We will start our analysis with the 2015 data. First, we can visualize this data set by first plotting points by geographic position.

```{r}
home.2015.dat <- read.csv("./rectangles/home.2015.csv")
plot(Northing ~ Easting, data=home.2015.dat,pch='.')
```
...
For example, we might divide a cropland into four quadrants. This requires determining a mid-point for `Northing` and a mid-point for `Easting`. We are working with yield maps where the GPS coordinates have been converted to metric distance from a common origin, so for these data we can assume the lower left corner of the cropland is at $\{0,0\}$. 

For simplicity, we'll trim the data to lie within 400m north and 600 meast of the origin.

```{r}
home.2015.dat <- home.2015.dat[home.2015.dat$Northing<=400,]
home.2015.dat <- home.2015.dat[home.2015.dat$Easting<=600,]
```
...
NorthingRng <- 400
EastingRng <- 600
home.2015.dat$Row <- ceiling(2*home.2015.dat$Northing/NorthingRng)
home.2015.dat$Col <- ceiling(2*home.2015.dat$Easting/EastingRng)
```
...
plot(Northing ~ Easting, data=home.2015.dat,col=grid.colors[as.character(home.2015.dat$Cell)])
```
...
for (i in 1:length(GridSummary$Divisions)) {
  ...
  #columns.
  div <- 2^i
  home.2015.dat$Row <- ceiling(div*home.2015.dat$Northing/NorthingRng)
  home.2015.dat$Col <- ceiling(div*home.2015.dat$Easting/EastingRng)
...
```{r}
home.2015.dat$Row <- ceiling((2^4)*home.2015.dat$Northing/NorthingRng)
home.2015.dat$Col <- ceiling((2^4)*home.2015.dat$Easting/EastingRng)
home.2015.dat$Cell <-home.2015.dat$Row*1000 + home.2015.dat$Col
```
...
plot(Northing ~ Easting, data=home.2015.dat,col=grid.colors[home.2015.dat$Check])
```
...
plot(Northing ~ Easting, data=sub.dat, col=grid.colors[sub.dat$Check])
```
...
for (i in 1:length(GridSummary$Rows)) {
  home.2015.dat$Row <- ceiling(GridSummary$Rows[i]*home.2015.dat$Northing/NorthingRng)
  home.2015.dat$Col <- ceiling(6*home.2015.dat$Easting/EastingRng)
  home.2015.dat$Cell <-home.2015.dat$Row*1000 + home.2015.dat$Col
...
```{r}
home.2015.dat$Row <- ceiling(20*home.2015.dat$Northing/NorthingRng)
home.2015.dat$Col <- ceiling(6*home.2015.dat$Easting/EastingRng)
home.2015.dat$Cell <-home.2015.dat$Row*1000 + home.2015.dat$Col
...
sub.dat <- home.2015.dat[home.2015.dat$Row<=2 & home.2015.dat$Col <= 3, ]
names(grid.colors) <- unique(sub.dat$Cell)
plot(Northing ~ Easting, data=sub.dat, col=grid.colors[as.character(sub.dat$Cell)])
```
...
for(i in 1:length(years)) {
  fields[[i]] <- read.csv(file=paste("./rectangles/home",years[i],"csv",sep="."))
  fields[[i]] <- fields[[i]][fields[[i]]$Northing<=400,]
  fields[[i]] <- fields[[i]][fields[[i]]$Easting<=600,]
}
```
...
```{r}
sample.over.rows <- function(harvest.dat,range=c(6,40),columns=6,NorthingRng=400,EastingRng=600) {
  rows <- 40
  GridSummary <- data.frame(Rows=6:rows)
  GridSummary$MinSamples=NA #Minimum number of samples
  GridSummary$Cells=NA      #total number of cells
  ...
  for (i in 1:length(GridSummary$Rows)) {
    harvest.dat$Row <- ceiling(GridSummary$Rows[i]*harvest.dat$Northing/NorthingRng)
    harvest.dat$Col <- ceiling(6*harvest.dat$Easting/EastingRng)
...
for(i in 1:length(fields)) {
  CurrentSummary <- sample.over.rows(fields[[i]])
  fields[[i]]$Row <- ceiling(20*fields[[i]]$Northing/NorthingRng)
  fields[[i]]$Col <- ceiling(6*fields[[i]]$Easting/EastingRng)
  fields[[i]]$Cell <-fields[[i]]$Row*1000 + fields[[i]]$Col
  fields[[i]]$Check <- (fields[[i]]$Row+fields[[i]]$Col) %% 2 + 1
  plot(Northing ~ Easting, data=fields[[i]],col=grid.colors[fields[[i]]$Check],
